...
import com.fruitshop.mapper.TbItemDescMapper;
...
public class ItemServiceImpl implements ItemService {
	...
	@Override
	public FruitshopResult getItemBaseInfo(long itemId) {
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return FruitshopResult.ok(item);
			}
	...
	@Override
	public FruitshopResult getItemDesc(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return FruitshopResult.ok(itemDesc);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public FruitshopResult getItemParam(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return FruitshopResult.ok(paramItem);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		return FruitshopResult.build(400, "无此商品规格");
	}
